...
$('.trigger').click(function () {
  ...
  $('.logo img, .portfolio_home__title h1, .portfolio_home__title hr, .portfolio_home__title img.trigger, .bars').addClass('out');
  $('.slider_inner').addClass('in');
  setTimeout(function () {
    $('.slider_inner').click();
    $('#subframe').remove();
    $(document).ready(function() {
      if(document.title === "PAWRes「ホーム」")
        document.title = 'PAWRes について';
    });
...
$('.slider_inner').mousemove(function (e) {
  ...
  if (dragging) {
    ...
    var position = parseInt($('.slider_inner').css('transform').split(',')[4]);
...
$('.slider_inner').mouseup(function () {
  ...
  if (difference < -160) {
    ...
    if (index < 3) {
      index++;
      threshold = offset - (offset + margin) * index;
